...
for dil in bot.iter_messages(pchannel, filter=InputMessagesFilterDocument):
    if ((len(dil.file.name.split(".")) >= 2) and (dil.file.name.split(".")[1] == "Lostjson")):
        if path.isfile(f"./userbot/language/{dil.file.name}"):
            try:
                ...
                if path.isfile("./userbot/language/DEFAULT.Lostjson"):
                    LOGS.warn("Varsayılan dil dosyası kullanılıyor...")
                    LANGUAGE_JSON = loads(open(f"./userbot/language/DEFAULT.Lostjson", "r").read())
                else:
...
if LANGUAGE_JSON == None:
    if path.isfile(f"./userbot/language/{LANGUAGE}.Lostjson"):
        try:
            LANGUAGE_JSON = loads(open(f"./userbot/language/{LANGUAGE}.Lostjson", "r").read())
        except JSONDecodeError:
