...
async def dil(event):
    ...
    if search(r"y[uü]kle|install", komut):
        ...
        if event.is_reply:
            ...
            if ((len(reply.file.name.split(".")) >= 2) and (not reply.file.name.split(".")[1] == "Lostjson")):
                return await event.edit("`Lütfen geçerli bir` **LostJSON** `dosyası verin!`")
        ...
        if event.is_reply:
            reply = await event.get_reply_message()
            if ((len(reply.file.name.split(".")) >= 1) and (not reply.file.name.split(".")[1] == "Lostjson")):
                return await event.edit("`Lütfen geçerli bir` **LostJSON** `dosyası verin!`")
        ...
        await event.edit(
            ...
            f"\n\nDiğer diller için @LostDil kanalına bakabilirsiniz."
        )
